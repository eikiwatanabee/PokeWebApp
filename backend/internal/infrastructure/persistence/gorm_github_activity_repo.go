...
func (r *GormGitHubActivityRepository) FindRecentByTenantID(ctx context.Context, tenantID uuid.UUID, limit int) ([]*entity.GitHubActivity, error) {
	tx := GetTx(ctx, r.db)
	var models []GitHubActivityModel
	q := tx.Where("user_id IN (SELECT id FROM users WHERE tenant_id = ?)", tenantID).Order("created_at DESC")
	if limit > 0 {
		q = q.Limit(limit)
	}
	if err := q.Find(&models).Error; err != nil {
...
func (r *GormGitHubActivityRepository) CountByUserIDSince(ctx context.Context, userID uuid.UUID, since time.Time) (int64, error) {
	tx := GetTx(ctx, r.db)
	var count int64
	if err := tx.Model(&GitHubActivityModel{}).Where("user_id = ? AND created_at >= ?", userID, since).Count(&count).Error; err != nil {
		return 0, err
	}
	return count, nil
}

func (r *GormGitHubActivityRepository) CountByUserIDAndTypeSince(ctx context.Context, userID uuid.UUID, eventType entity.GitHubEventType, since time.Time) (int64, error) {
	tx := GetTx(ctx, r.db)
	var count int64
	if err := tx.Model(&GitHubActivityModel{}).Where("user_id = ? AND event_type = ? AND created_at >= ?", userID, string(eventType), since).Count(&count).Error; err != nil {
		return 0, err
	}
	return count, nil
}

func (r *GormGitHubActivityRepository) TotalXPByUserIDSince(ctx context.Context, userID uuid.UUID, since time.Time) (int, error) {
	tx := GetTx(ctx, r.db)
	var total *int
	if err := tx.Model(&GitHubActivityModel{}).Where("user_id = ? AND created_at >= ?", userID, since).Select("COALESCE(SUM(xp), 0)").Scan(&total).Error; err != nil {
		return 0, err
	}
	if total == nil {
		return 0, nil
